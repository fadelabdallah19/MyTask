...
<div class="min-h-screen bg-gray-50">
    ...
    <main class="max-w-3xl mx-auto p-4 sm:p-6">
        ...
        <h2 class="text-2xl font-bold text-gray-800 mb-6">Buat Task Baru</h2>
        ...
        <form method="POST" action="{{ route('tasks.store') }}" class="bg-white rounded-lg shadow p-6 space-y-4">
            @csrf

            <div>
                <label for="title" class="block text-sm font-medium text-gray-700 mb-1">Judul</label>
                <input type="text" id="title" name="title" value="{{ old('title') }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="datetime-local" id="deadline" name="deadline" value="{{ old('deadline') }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>

            <div class="pt-2">
                <button type="submit" class="w-full bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 transition-colors">Simpan Task</button>
            </div>
        </form>
    </main>
</div>
